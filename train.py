...
class ESC50Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            spectorgram= self.transform(waveform)
        else:
            spectorgram=waveform

        return spectorgram, row['label']
...
def mixup_data(x,y):
    ...
    mixed_x= lam * x + (1-lam)*x[index,:]       #70-30% mix
    y_a, y_b=y, y[index]        #y_a being label of the first in the mixture and y_b being the second
    ...
    for epoch in range(num_epochs):
        ...
        for data, target in progress_bar:
            ...
            if np.random.random()>0.7:
                ...
                output=model(data)
                loss=mixup_loss(criterion,output, target_a, target_b,lam)
            else:
                output=model(data)
