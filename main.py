...
class AudioClassifier:
    ...
    def inference(self, request:InferenceRequest):
        ...
        if audio_data.ndim>1:
            audio_data=np.mean(audio_data, axis=1)      #Convert to mono-audio
        ...
        with torch.no_grad():
            ...
            for name,tensor in feature_maps.items():
                if tensor.dim()==4:     #[batch_size,channels,height, width]
                    aggregrated_tensor= torch.mean(tensor, dim=1)
                    squeezed_tensor=aggregrated_tensor.squeeze(0)
...
def main():
    audio_data, sample_rate=sf.read("Slurping.wav")
