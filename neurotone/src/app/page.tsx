...
export default function HomePage() {
  ...
  const handleFileChange= async(event:React.ChangeEvent<HTMLInputElement>)=>{
    ...
    reader.onload=async ()=>{
      ...
      const response=await fetch("https://saransh1578--cnn-audio-audioclassifier-inference.modal.run/",{
        method:"POST",
  ...
  return (
    <main className="min-h-screen bg-stone-300 p-8">
      <div className="mx-auto max-w-[100%]">
        <div className="mb-12 text-center">
          <h1 className="mb-4 text-4xl font-light tracking-tight text-stone-900">
            NeuroTone
          </h1>
          <p className="mb-8 text-lg text-slate-700">Upload a WAV file to see the model's predictions...</p>
          ...
          <div className="flex flex-col items-center">
            <div className="relative inline-block">
              ...
              />
              <Button disabled={isLoading} className="border-stone-600" variant="outline" size="lg">
                {isLoading? "Analyzing...":"Choose a File."}
              </Button>
            </div>
            {fileName &&(<Badge variant="secondary" className="text-stone-700 mt-4 bg-stone-500">{fileName}</Badge>)}
          </div>
        </div>
        {error && (
        ...
        {vizData && (<div className="space-y-8">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <Card >
              ...
              <CardContent>
                <FeatureMaps data={vizData.input_spectogram.values} title={`${vizData.input_spectogram.shape.join(" x ")}`} spectrogram></FeatureMaps>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-stone-900">Convolutional Layer Outputs</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                {main.map(([mainName,mainData])=>(
                  <div key={mainName} className="space-y-4">
                    <div>
                      ...
                      <FeatureMaps data={mainData.values} title={`${mainData.shape.join(" x ")}`}/>
                    </div>
                    {internals[mainName] && <div className="h-80 overflow-y-auto rounded border border-stone-200 bg-stone-50 p-2">
                      <div className="space-y-2">
                        {internals[mainName].sort(([a],[b])=>
                          a.localeCompare(b))
                          .map(([layerName,layerData])=>(
                            <FeatureMaps key={layerName} data={layerData.values} title={layerName.replace(`${mainName}.`,"")} internal={true}/>
                          ))}
                      </div>
                    </div>}
                  </div>
